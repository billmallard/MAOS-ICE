...
| Boost motor + controller (EMRAX 228-class) | 28 kg | $10,000 | $5,000 | **EXTRAP** — scaled from EMRAX 268's SOURCED 30.4 kg/$17,000 by power ratio; EMRAX 228's own continuous rating was **not independently re-verified this pass** |
...
| Climb (1,000 fpm anchor, EXTRAP scaling): ICE + boost motor combined | 195.6 kW needed vs. ~196 kW available (146 + ~50 kW motor boost, EXTRAP) — **~0.2% margin, razor-thin and resting on an unverified motor rating** | similar, slightly worse |

**Reserve after ICE loss:** ICE-out gives a real powered descent (~26 kWh at 40 kW, pending AERO's profile) — a genuine capability A0 does not have. **Motor/inverter-out:** ICE continues driving the prop unassisted (per the architecture's own definition — two independent torque paths on one shaft), losing boost/climb margin but not thrust. **Not yet resolved:** whether this shared-shaft arrangement satisfies AER-81 R1's redundancy intent (written for an all-electric twin-motor case) — SAFETY consult opened separately (§6), not self-certified here. **Also not resolved:** a double-fault case (battery/bus failure first, silently erasing the reserve, followed later by ICE failure) leaves A1 in exactly A0's unpowered-glide case with no warning that the safety margin is gone — a real finding for SAFETY's FMEA, not previously named in this form.
...
- **STRUCTURES**: airframe-only empty weight (excl. powertrain/fuel/crew/payload) for the post-ORG-003 airplane. Every MTOW in this document is provisional on this number (§0.4).
...
- **SAFETY**: AER-81 R1 was ruled for an all-electric twin-motor case; whether A1's ICE+motor-on-one-shaft arrangement satisfies its redundancy intent is a distinct question, opened as a separate consult, not self-certified in this document.
