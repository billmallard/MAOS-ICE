...
| HV bus voltage (DG-004) | SYSTEMS / PROPULSION, joint | **Open.** `COMBO3_CORRECTED_ELECTRIC_MACHINE_V1.md` §2 proposes ~700–800 Vdc pending Systems' concurrence; per the 2026-08-30 weekly status, "no settled number or explicit concurrence request has reached SYSTEMS" formally. This document uses the proposed ~700–800 Vdc as its working bus-voltage assumption (ASSUM) and requests SYSTEMS close DG-004 against it — see Open Issues. |
| Mount datum, envelope, load path (aft pod / boom root) | STRUCTURES / AER-69 | **Consulted, not yet returned.** AER-69 v2's live CAD places the motor/generator assembly at the boom root, coincident with the pod's aft structure — the same volume a pusher engine would occupy (`propulsion_reset.md` §2.4). No firewall/nacelle envelope dimensions (length, diameter, mount-face location) were found in the design-state documents; this ICD specifies the mounting **pattern**, not a fixed envelope, and names the exact STRUCTURES pull needed in Open Issues. |
...
| Envelope | **Not yet fixed — see Open Issues.** AER-69 v2 places the assembly at the boom root / pod aft structure; exact length/diameter/mount-face-to-firewall dimensions were not found in any design-state document reviewed for this draft. | **Blocked on STRUCTURES** |
...
- **Power source and bus:** the genset feeds the MAOS HV DC bus (DG-004). **Nominal voltage: ~700–800 Vdc (open, pending SYSTEMS concurrence — see Open Issues).**
...
- **Transport:** CAN bus, MAOS internal message format (native on a Teensy 4.1-class Speeduino build, per `ENGINE_MGMT_CANDIDATE_MATRIX_V1.md` §3.3; requires a bridge for MegaSquirt MS3-Pro's proprietary protocol or an analog-signal bridge for SDS EM-6, which has no native CAN).
...
|---|---|---|---|
| **Overboost** | ICE-FLT-001 (extended) | Manifold absolute pressure exceeds the tune's target boost by a defined margin (ASSUM: +15% of target gauge boost) sustained beyond a debounce window (ASSUM: 500 ms, to reject transient spikes) | Derate: MAOS/ECU commands wastegate toward open / cuts boost-control duty cycle; if MAP does not recover within a bounded window, shutdown |
...
| # | Issue | Owner | Action |
|---|---|---|---|
| 1 | DG-004 (HV bus voltage) still open — no settled number has formally reached SYSTEMS for concurrence | SYSTEMS / PROPULSION | This document uses ~700–800 Vdc as its working assumption; requests SYSTEMS post explicit concurrence or a counter-proposal on this issue |
| 2 | STRUCTURES has not returned a firewall/nacelle envelope (length, diameter, mount-face location) for the boom-root/pod-aft installation | STRUCTURES | Requested on this issue; this ICD specifies a mounting *pattern*, not a fixed envelope, until this lands |
| 3 | Torsional/vibration analysis of the direct-coupled generator shaft not performed | PROPULSION | Named as a pre-hardware-commitment gate in §1 |
